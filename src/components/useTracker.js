// taken from https://github.com/meteor/react-packages/blob/devel/packages/react-meteor-data/useTracker.js
/* global __DEV__ */
...
const runReactiveFn = isDevelopment
  ? (refs, c) => {
...
const tracked = (refs, c, forceUpdate) => {
  if (c.firstRun) {
    ...
    if (refs.computationHandler) {
      ...
      if (cleanupHandler) {
        if (isDevelopment && typeof cleanupHandler !== 'function') {
          warn(
